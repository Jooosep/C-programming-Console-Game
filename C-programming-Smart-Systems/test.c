...
void thunder(int progress)
{
	...
	for (int i = 0; i < thunderVolume; i++)
	{
		...
		if (thunderLaunchPoints[i] < 1)
		{
			thunderLaunchPoints[i] = MAX_X + thunderLaunchPoints[i];
		}
		else if (thunderLaunchPoints[i] > MAX_X - 1)
		{
	...
	for (int i = 0; i < thunderVolume; i++)
	{
		...
		else if (progress > 47)
		{
			...
			{
				if (r[i] < 34 && travelPoints[i] < MAX_X - 1)
				{
					travelPoints[i] += 1;
					recordPath(i, 0);
				}
				else if (r[i] < 67 && travelPoints[i] > 1)
				{
					travelPoints[i] -= 1;
					recordPath(i, 1);
				}
...
int gameProgress()
{
	...
	{
		thunderSpeed--;
		thunderFrequency = thunderFrequency - 80;
	}
	else if (points > 24 && thunderVolume == 1)
	{
		gotoxy(80, 30);
	...
	else if (points > 34 && thunderVolume == 2)
	{
		gotoxy(80, 30);
		printf("Choose wisely, things will only get worse from here");
		for (int i = 0; i < 4; i++)
	...
	else if (points > 49 && thunderVolume == 3)
	{
		gotoxy(80, 30);
		printf("Choose wisely, things will only get worse from here");
		for (int i = 0; i < 4; i++)
	...
	else if (points > 69 && thunderVolume == 4 && thunderSpeed > 7)
	{
		thunderSpeed = 7;
	...
	else if (thunderSpeed > 5 && points > 99)
	{
		gotoxy(80, 30);
		printf("Choose wisely, things will only get worse from here");
		gotoxy(80, 31);
	...
	else if (thunderSpeed > 3 && points > 129)
	{
		thunderFrequency = thunderFrequency - 300;
		thunderVolume = 5;
		thunderSpeed = 3;
	...
	else if (points > 159 && thunderVolume == 5)
	{
		timeSlowPerk = 0;
		thunderVolume = 6;
		thunderFrequency -= 300;
	}
	else if (points > 199 && progressiveMode)
	{
		system("CLS");
...
int intro()
{
	...
	gotoxy(50, 45);
	hidecursor();
	printf("PRESS 1 AND ENTER TO PLAY NORMAL MODE OR 2 AND ENTER FOR SURVIVAL MODE");
	c = askNumber();
...
main()
{
	...
	while (1)
	{
		...
		for (int i = 0; i < 5; i++)
		{
			if (treeFruit[i] == 1)
			{
				if (abs(treePosition[i] - currentPosition) < 4)
				{
